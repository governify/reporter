...
const atoms = {
  ...
  gaugeLast5DaysNormalizedNotZero: {
    title: 'Mean percentage since the beginning',
    guarantee: '###GUARANTEE.NAME###',
    type: 'gauge',
    datasource: 'InfluxDB',
    ...
    targets: [
      {
        ...
        orderByTime: 'ASC',
        policy: 'autogen',
        query: "SELECT count(\"guaranteeValue\") AS \"valid\" FROM \"autogen\".\"metrics_values\" WHERE (\"agreement\" = '###AGREEMENT.ID###' AND \"id\" = '###GUARANTEE.NAME###' AND \"###METRIC.NOTZERO###\" != 0 AND \"guaranteeValue\" ###GUARANTEE.OPERATOR### ###GUARANTEE.THRESHOLD###) AND time < now() AND time >= now() -500d GROUP BY time($__interval) fill(null)",
        rawQuery: true,
        refId: 'A',
        resultFormat: 'time_series',
        select: [
      ...
      },
      {
        refId: 'B',
        hide: false,
        query: "SELECT count(\"guaranteeValue\") AS \"total\" FROM \"autogen\".\"metrics_values\" WHERE (\"agreement\" = '###AGREEMENT.ID###' AND \"id\" = '###GUARANTEE.NAME###' AND \"###METRIC.NOTZERO###\" != 0) AND time < now() AND time >= now() -500d GROUP BY time($__interval) fill(null)",
        rawQuery: true
      }
    ],
    transformations: [
  ...
  gaugeLastPeriodNormalizedNotZero: {
    title: 'Period mean percentage',
    guarantee: '###GUARANTEE.NAME###',
    type: 'gauge',
    datasource: 'InfluxDB',
    ...
    targets: [
      {
        ...
        orderByTime: 'ASC',
        policy: 'autogen',
        query: "SELECT count(\"guaranteeValue\") AS \"valid\" FROM \"autogen\".\"metrics_values\" WHERE (\"agreement\" = '###AGREEMENT.ID###' AND \"id\" = '###GUARANTEE.NAME###' AND \"###METRIC.NOTZERO###\" != 0 AND \"guaranteeValue\" ###GUARANTEE.OPERATOR### ###GUARANTEE.THRESHOLD###) AND $timeFilter GROUP BY time($__interval) fill(null)",
        rawQuery: true,
        refId: 'A',
        resultFormat: 'time_series',
        select: [
      ...
      },
      {
        refId: 'B',
        hide: false,
        query: "SELECT count(\"guaranteeValue\") AS \"total\" FROM \"autogen\".\"metrics_values\" WHERE (\"agreement\" = '###AGREEMENT.ID###' AND \"id\" = '###GUARANTEE.NAME###' AND \"###METRIC.NOTZERO###\" != 0) AND $timeFilter GROUP BY time($__interval) fill(null)",
        rawQuery: true
      }
    ],
    transformations: [
  ...
  timeGraphPercentNotZeroWithoutMean: {
    title: '###TIME_GRAPH.TITLE###',
    guarantee: '###GUARANTEE.NAME###',
    type: 'graph',
    aliasColors: {},
...
const blocks = {
  ...
  'time-graph2-member-normalized-notZero': {
    config: {
      height: 8
    },
    panels: [
      addAtom('rowTitle'),
      addAtom('gaugeLast5DaysNormalizedNotZero', 3, 6, 0),
      addAtom('gaugeLastPeriodNormalizedNotZero', 4, 9, 3),
      addAtom('timeGraphMemberNotZero', 17, 9, 7)
    ]
  },
  ...
  'time-graph-team-not-zero': {
    ...
    panels: [
      ...
      addAtom('gaugeLastPeriodNotZero', 4, 9, 3),
      addAtom('timeGraphPercentNotZeroWithoutMean', 17, 9, 7),
    ]
  },
...
module.exports.default = (jsonDashboard, agreement, dashboardName) => {
  ...
  Object.entries(dashboardConfig.blocks).sort((a, b) => a[0] - b[0]).forEach(function ([_, block]) {
    ...
    if (block.type === 'correlated' || block.type === 'gauge-time-correlation' || block.type === 'gauge-time-correlation-notZero' || block.type === 'gauge-period-time-correlation-notZero' || block.type === 'gauge-period-time-correlation-notZero-member' || block.type === 'time-graph-team-not-zero') {
    ...
    if (block.type === 'time-graph2-member' || block.type === 'time-graph-team' || block.type === 'time-graph2-member-notZero' || block.type === 'time-graph2-member-groupby' || block.type === 'time-graph-count' || block.type === 'time-graph-count-groupby' || block.type === 'gauge-period-time-correlation-notZero' || block.type === 'gauge-period-time-correlation-notZero-member' || block.type === 'gaugeLast5DaysNormalized' || block.type === 'time-graph-team-not-zero' || block.type === 'time-graph2-member-normalized-notZero') {
      ...
      if(block.type === 'time-graph2-member' || block.type === 'time-graph2-member-groupby' || block.type === 'time-graph-team' || block.type === "time-graph2-member-normalized-notZero") {
